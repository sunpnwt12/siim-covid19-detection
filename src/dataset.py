...
class SiimDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transforms:
            transforms_dict = {"image": image, "mask": mask, "class_labels": target}
...
test_df_path = Path(cfg.repo_dir) / "dataset" / "test_df_local.csv"
...
def get_valid_transforms():
    return A.Compose([A.Resize(cfg.image_size, cfg.image_size), ToTensorV2()])
...
def get_train_dataset(fold):
    return SiimDataset(
        df=folds_df[folds_df["fold"] != fold], transforms=get_train_transforms()
    )


def get_valid_dataset(fold):
    return SiimDataset(
        df=folds_df[folds_df["fold"] == fold], transforms=get_valid_transforms()
    )


def get_test_dataset():
    return SiimDataset(df=test_df, transforms=get_test_transforms())
